<html>
	...
	<h2> Selection on user selected table and attribute-value pair </h2>

        <form method="GET" action="project.php"> <!--refresh page when submitted-->
            
            Which table are we making the selection on? <br><br>
            ...
            Which attributes-values are we making the selection on? <br><br>
            Attribute 1: <br><br>
            <select name="Attribute1" id="Attribute1">
                <option value="CityCoords">CityCoords</option>
                <option value="AgencyName">AgencyName</option>
                <option value="HotelName">HotelName</option>
                <option value="HotelLocation">HotelLocation</option>
	...
	<h2>Find the number of customers visiting all Cities that have been visited (Group By)</h2>
        
        <form method="GET" action="project.php"> <!--refresh page when submitted-->
            <input type="hidden" id="attractionsCountRequest" name="attractionsCountRequest">
            <input type="submit" name="attractionsCountSubmit"></p>
        </form>
	...
	<h2> Find Customers Who Have Stayed at All Hotels (Division)</h2>

        <form method="GET" action="project.php"> <!--refresh page when submitted-->
            <input type="hidden" id="findAllHotelsRequest" name="findAllHotelsRequest">
            <input type="submit" name="findAllHotelsSubmit"></p>
        </form>
	...
	/**
             * Select on the table selected by the user using user selected two attribute-value pairs
             */
            function handleSelectionRequest(){
                ...
                $result = executePlainSQL(
                    "SELECT *
                    FROM " . $_GET['selectedTable'] . 
                    " WHERE " .  $_GET['Attribute1'] . " = '" . $_GET['Value1'] . "' AND " . $_GET['Attribute2'] . " > " . $_GET['Value2']
                );
	     ...
	     /**
             * Returns the a list of all visited cities with the number of customers who visited each one of them
             */
            function handleAttractionsCountRequest(){
                global $db_conn;
                
                $result = executePlainSQL(
                "SELECT Count(ID), CityCoords
                FROM TravelCity
                GROUP BY CityCoords"
                );
                ...
                while($row = oci_fetch($result)){
                    echo "<br> " . oci_result($result, 'COUNT(ID)') . " Customer(s) visited city with CityCoords: "
                    . oci_result($result, 'CITYCOORDS') .  "<br>";
                }
	...
	function handleFindAllHotelsRequest(){
                global $db_conn;
                ...
                $result = executePlainSQL(
                    ...
                    FROM Customer c
                    WHERE NOT EXISTS
                            ((SELECT h.HotelName, h.HotelLocation
                            FROM Hotel h)
                            MINUS
                            (SELECT hr.HotelName, hr.HotelLocation
                            FROM HotelReservation hr
                            WHERE hr.ID = c.ID))"
                );
		
		echo "<br>Customer(s) who has a reservation at all available hotels:<br>";
                echo "<table>";
                echo "<tr><th>CustomerName</th><th>ID</th></tr>";
                
                while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                    echo "<tr><td>" . $row["CUSTOMERNAME"] . "</td><td>" . $row["ID"] . "</td></tr>"; //or just use "echo $row[0]"
                }
        ...
        function handleGETRequest() {
            if (connectToDB()) {
                ...
                } else if (array_key_exists('attractionsCountSubmit', $_GET)) {
                    handleAttractionsCountRequest();
                ...
                }else if (array_key_exists('findAllHotelsSubmit', $_GET)) {
                    handleFindAllHotelsRequest();
        ...
        isset($_GET['attractionsCityRequest']) || isset($_GET['selectCityRequest']) || isset($_GET['selectionRequest']) || isset($_GET['hotelCountRequest']) || isset($_GET['attractionsCountRequest']) || isset($_GET['luggageRequest']) || 
        isset($_GET['attractionTourRequest']) || isset($_GET['expensiveHotelRatesRequest']) || isset($_GET['findAllHotelsRequest'])|| isset($_GET['allTripsRequest'])) {
            handleGETRequest();
        }
        ?>
    </body>
